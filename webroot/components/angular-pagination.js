angular.module('pagination', [])
  .directive('paginate', function( $timeout, $location) {
    return {
        restrict: 'C',
        replace: true,
        scope: {
        ...
        controller: function($scope, $element, $attrs) {
            ...
            $scope.appendItem = function(pageIndex, opts) {
                ...
                if (pageIndex == $scope.currentPage) {
                    ...
                    link = $('<a href="#' + url + '?page=' + pageIndex + '" class="page-link">' + (options.text) + '</a>');
            ...
            $scope.draw = function() {
                ...
                // Generate Prev link
                if (true) {
                    $scope.appendItem($scope.currentPage - 2, {
                        text: 'Prev',
                        classes: 'prev'
                    });
                }
                ...
                // Generate Next link
                if (true) {
                    $scope.appendItem($scope.currentPage, {
                        text: 'Next',
                        classes: 'next'
                    });
                }
  ...
  .factory('paginationFactory', function( $routeParams) {
    return {
      url: function( url) {
        if( $routeParams.page) {
